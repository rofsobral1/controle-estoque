...
export default function ListarFornecedoresPage() {
  ...
  const handleUpdate = async () => {
    if (!formData) return;
    try {
      await apiFetch(`/fornecedores/${formData.id}`, {
        method: "PUT",
    ...
    >
      ...
      <div className="flex-1 flex flex-col relative z-10">
        <header className="p-4 flex items-center justify-between lg:hidden">
          ...
          <h1 className="text-lg font-bold text-white">Listagem de Fornecedores</h1>
        </header>
        ...
        <main className="p-8 flex-1 relative">
          ...
          {loading ? (
            <p className="text-white text-sm text-center">Carregando fornecedores...</p>
          ) : fornecedores.length === 0 ? (
            <p className="text-white text-sm text-center">Nenhum fornecedor encontrado.</p>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {fornecedores.map((fornecedor) => (
                ...
                >
                  <div>
                    <h2 className="text-lg font-semibold text-white mb-1">{fornecedor.nome}</h2>
                    <p className="text-gray-200"><strong>CNPJ:</strong> {fornecedor.cnpj}</p>
                    <p className="text-gray-200"><strong>Telefone:</strong> {fornecedor.telefone}</p>
                    <p className="text-gray-200"><strong>Email:</strong> {fornecedor.email}</p>
                    <p className="text-gray-200"><strong>Endereço:</strong> {fornecedor.endereco}</p>
                    <p className="text-gray-200"><strong>CEP:</strong> {fornecedor.cep}</p>
                    <p className="text-gray-200"><strong>Bairro:</strong> {fornecedor.bairro}</p>
                    <p className="text-gray-200"><strong>Estado:</strong> {fornecedor.estado}</p>
                    <p className="text-gray-200"><strong>Data Inclusão:</strong> {new Date(fornecedor.dataInclusao).toLocaleDateString()}</p>
                  </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-[rgba(15,32,39,0.85)] rounded-2xl shadow-xl w-full max-w-3xl p-6 text-white backdrop-blur-md">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {["nome","cnpj","telefone","email","endereco","cep","bairro","estado"].map((field) => (
                <div key={field}>
