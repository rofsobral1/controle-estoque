...
export default function CadastrarClientePage() {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const numericValue = ["telefone", "cpf", "cnpj", "cep"].includes(name)
      ? value.replace(/\D/g, "")
      : value;
    ...
    if (name === "cpf") {
      setFormData((prev) => ({
        ...prev,
        cpf: numericValue,
        cnpj: numericValue ? "" : prev.cnpj,
      }));
    } else if (name === "cnpj") {
      setFormData((prev) => ({
        ...prev,
        cnpj: numericValue,
        cpf: numericValue ? "" : prev.cpf,
      }));
    } else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const { cpf, cnpj, ...rest } = formData;
    const hasEmptyField = Object.values(rest).some((value) => !value.trim());

    if (hasEmptyField) {
      setError("Todos os campos devem ser preenchidos (exceto CPF ou CNPJ).");
      setLoading(false);
      return;
    }
    if (!cpf && !cnpj) {
            ...
            <div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <input
                  type="text"
                  name="cpf"
                  value={formatValue("cpf", formData.cpf)}
                  onChange={handleChange}
                  placeholder="CPF"
                  disabled={!!formData.cnpj}
                  className={`w-full p-3 rounded-lg text-white placeholder-gray-400 border border-white/30 text-sm placeholder:text-sm focus:ring-2 focus:ring-blue-400 ${
                    formData.cnpj
